...
| Scaffolding | CLI: `abi-core create project` + `add agent`/`add service` | Manual setup or minimal templates |
...
Everything is modular and extensible. `abi-core create project` scaffolds the base infrastructure. `abi-core add agent`/`add service` add pieces one at a time — an Orchestrator/Planner/Builder swarm is assembled from the same building blocks as a single agent, not a separate generator. TUI widgets are reusable. Jinja2 templates are customizable. Each piece works standalone or wired together.
...
### Create a Project
```bash
abi-core create project --name my-project --with-semantic-layer --with-guardian
cd my-project
abi-core add agent --name my-agent --description "What it does"
abi-core run
```

This scaffolds a project with Guardian, Semantic Layer, Weaviate, OPA, and Ollama wired up in Docker Compose, then adds your agent to it. `abi-core run` starts everything and launches the interactive TUI.

An Orchestrator + Planner + Builder swarm (ephemeral agents on demand, plan confirmation, task decomposition — the pipeline described below) isn't a single generated command; it's assembled the same way, one `add agent`/`add service` at a time, using the reference implementations in `abi_agents` (`abi_agents.orchestrator`/`.planner`/`.builder`) as the starting point. See [ABI Swarm](https://github.com/Joselo-zn/abi-swarm) for a maintained example of that assembly.
...
### runtime.yaml
Tracks the project's agents/services as they're added — `abi-core create project` writes the `project` section, and each `abi-core add agent`/`add service` call appends its own entry to `.abi/runtime.yaml`:
```yaml
project:
  name: "my-project"
  version: "1.0.0"
...
- [Architecture Overview](abi_core_framework/docs/reference/architecture.md)
