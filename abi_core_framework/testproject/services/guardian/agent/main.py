...
agent_card_dir = os.getenv('AGENT_CARD', './agent/agent_cards/guardian_agent.json')
...
def guardian_factory(agent_card_dir: AgentCard):
    ...
    if agent_card.name != "testproject Guardian":
...
async def secure_startup():
    ...
    try:
        ...
        server_thread = threading.Thread(
            target=start_server,
            args=("0.0.0.0", 11438, agent_card_dir, guardian_factory),
            daemon=False
        )
        server_thread.start()
...
if __name__ == '__main__':
    ...
    from abi_core.opa.policy_loader_v2 import PolicyLoaderV2, get_policy_loader
    from abi_core.opa.core_policies import CorePolicyGenerator, get_core_policy_generator
    ...
    # Use opa_config to avoid overwriting the GuardianConfig instance
