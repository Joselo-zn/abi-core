...
3. **Initialize Ollama models:**
   ```bash
   docker exec testproject-ollama ollama pull qwen2.5:3b
   
   docker exec testproject-ollama ollama pull nomic-embed-text:v1.5
...
|----------|---------|-------------|
| `ABI_ROLE` | testproject | ABI role identifier |
...
2. **Ollama models not available:**
   ```bash
   docker exec testproject-ollama ollama list
   docker exec testproject-ollama ollama pull [model_name]
   ```
...
4. **Policy evaluation errors:**
   ```bash
   # Check OPA logs
   docker-compose logs testproject-opa
   
   # Test policy syntax
   docker exec testproject-opa opa test /policies
   ```


### Support
- **Documentation**: [ABI-Core Docs](https://github.com/Joselo-zn/abi-core)
- **Issues**: [GitHub Issues](https://github.com/Joselo-zn/testproject/issues)
