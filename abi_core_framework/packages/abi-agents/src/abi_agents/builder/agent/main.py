#!/usr/bin/env python3
"""Builder Agent — Generated by ABI-Core

DAG enforces the 5-step build process deterministically:

  parse_spec → verify_tools → generate_config → build_container → register_card

Each step maps to a prompt step + rules:
  Step 1 (Analyze Spec)    → parse_spec      | Rule 6: partial progress on error
  Step 2 (Resolve Tools)   → verify_tools    | Rule 1: always verify before build
  Step 3 (Build Agent)     → generate_config | Rule 2: unique names (task_id+ts)
                            → build_container | Rule 3: inherit LLM config
  Step 4 (Report Back)     → register_card   | Rule 4: ephemeral=true
  Step 5 (Cleanup)         → handled by orchestrator signal
  All steps                                  | Rule 5: abi_logging every step
"""
