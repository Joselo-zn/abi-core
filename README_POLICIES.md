...
ABI uses Open Policy Agent (OPA) for policy enforcement across the entire pipeline. The Guardian agent validates every request against OPA policies before execution. Immutable core policies are auto-generated at startup — agents cannot modify them. Custom policies can be added per project.

## Policy Sources (Priority Order)

1. **Built-in Policies** (Highest Priority)
   - Location: Guardian service `opa/policies/` directory
   - Generated by: `abi-core create swarm` or `abi-core add service guardian`
   - Contains: Core ABI security policies (`basic.rego`, `semantic_access.rego`, `a2a_access.rego`)
...
```json
{
    ...
    "source_agent": "orchestrator|planner|builder|guardian|zombie",
...
```python
from abi_core.opa.policy_engine import get_policy_engine
