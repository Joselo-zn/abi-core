...
from a2a.server.agent_executor import AgentExcutor, RequestContext
from a2a.server.events import EventQueue
from a2a.server.task import TaskUpdater
from a2a.utils import new_agent_text_message, new_task
from a2aa.utils.error import ServerError
...
class ABIAgentExecutor(AgentExcutor):
    """Execute ABI agents"""
    ...
    logger.info(f'Executing ABI AGENT {agent.agent_name}')
    ...
    async for item in self.agent.stream(query, task.content_id, task.id):
        ...
        requiere_user_input = item['requiere_user_input']
