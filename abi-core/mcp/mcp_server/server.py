...
def load_agent_cards():
    ...
    card_uris = []
    agent_cards = []
    BASE_DIR = Path(os.getenv("AGENT_CARDS_BASE")) if os.getenv("AGENT_CARDS_BASE") \
                else Path(__file__).resolve().parent
    AGENT_CARDS_DIR = Path(os.getenv("AGENT_CARDS_DIR", "/app/mcp_server/agent_cards"))

    dir_path = AGENT_CARDS_DIR if AGENT_CARDS_DIR.is_absolute() else (BASE_DIR / AGENT_CARDS_DIR)

    if not dir_path.is_dir():
        logger.error(f'[!] Agent card directory not found {dir_path}')
        return agent_cards
...
def bulding_agent_card_embeddings() -> pd.DataFrame:
    ...
    try:
        ...
        logger.error(f'[!] An unexpected error accourred: {e}', exc_info=True)
